...
Run:  python import_my_db_to_supabase.py
"""
...
KNOWN_ACCOUNTS = {"CORRENTE": "checking", "SANTANDER": "card",
                  "ITAU": "checking", "POUPANÇA": "savings"}

# ---------- cleaning helpers (mirror the validated transforms) ----------
def to_date(y, m, d):
    try: y, m, d = int(y), int(m), int(d)
    except (TypeError, ValueError): return None
    if not (1900 <= y <= 2100) or not (1 <= m <= 12): return None
    if d < 1 or d > 31: d = 1
    for dd in (d, 28, 1):
        try: return datetime.date(y, m, dd).isoformat()
...
def main():
    ...
    acc_id = upsert_lookup("accounts", KNOWN_ACCOUNTS.keys(),
    ...
    for r in src:
        ...
        tx.append({
            ...
            "created_at": clean_ts(r["created_at"]) or datetime.datetime.utcnow().isoformat(),
            "updated_at": clean_ts(r["last_update_at"]),
        })

    for i in range(0, len(tx), 500):           # batch to stay under limits
        sb.table("transactions").insert(tx[i:i + 500]).execute()
        print(f"  inserted {min(i + 500, len(tx))}/{len(tx)}")

    feelings = [{"feeling": r["Sentimento"], "sensation": r["Sensacao"], "emotion": r["Emocao"]}
    ...
    moods = [{"owner_id": OWNER, "feeling": r["sentimento"], "sensation": r["sensacao"],
              "emotion": r["emocao"], "occurred_at": clean_ts(r["datahora"]), "notes": r["detalhes"]}
             for r in cur.execute("SELECT * FROM tb_records")]
    if moods:
        sb.table("mood_entries").insert(moods).execute()
    print(f"done. transactions={len(tx)}, moods={len(moods)}")
    con.close()
